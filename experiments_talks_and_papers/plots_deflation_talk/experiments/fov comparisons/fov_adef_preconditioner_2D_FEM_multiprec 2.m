...
for i=1:length(kk)
    ...
    Aepsinv  =  precomputeLU(Aeps);
    AepsHinv =  precomputeLU(Aeps');
    time_lu = toc;
    ...
    M  = mp(eqn1.M);
    Ac = mp(Z'*A*Z); %Coarse operator
    
    fprintf('beginning computation of lu of coarse matrix for k=%d  \n', k);
    tic
    Acinv    = precomputeLU(Ac);
    AcHinv   = precomputeLU(Ac');
    time_luc = toc;
    ...
    APH =  @(x)  (AepsHinv\(M*x)) - PH(AepsHinv\(M*x));
