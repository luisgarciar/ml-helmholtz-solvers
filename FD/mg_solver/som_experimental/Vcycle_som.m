...
%%    
    if length(galerkin_matrices) == 1 %If on coarsest level, solve exactly
        ...
        for i=1:numcycles
            ...
            if(levs >2)
                vc  = Vcycle_som(galerkin_matrices(2:levs),galerkin_split(2:levs),restrict_op(2:levs-1),interp_op(2:levs-1),vc,fc,npre,npos,w,smo,1);
                %size(fc)
                
            elseif(levs==2)
                vc = Vcycle_som(galerkin_matrices(2:2),galerkin_split(2:2),restrict_op(1:1),interp_op(1:1),vc,fc,npre,npos,w,smo,1);
                %size(fc)
            end
