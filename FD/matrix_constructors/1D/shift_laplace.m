function [A] = shift_laplace(k,b1,b2,np,bc)
%% SHIFT_LAPLACE: Constructs matrices for the 1D Helmholtz problem.
%  Constructs the finite difference matrix corresponding
%  to the discretization of the 1D problem 
%          u''- k^2*(b1+i*b2)u = f
%
%  When homogeneous Dirichlet boundary conditions are used, 
%  the boundary points are eliminated from the linear system.
%  In case of Sommerfeld boundary conditions, the boundary
%  points are included.
%
%  Use: [A] = shift_laplace(k,b1,b2,np,bc)
%
%  Input: 
%  k:      wavenumber 
