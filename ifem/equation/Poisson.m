function [u,Du,eqn,info] = Poisson(node,elem,pde,bdFlag,option)
%% POISSON Poisson equation: P1 linear element.
%
%   u = Poisson(node,elem,pde,bdFlag) produces the linear finite element
...
% 
%   u = Poisson(node,elem,pde,bdFlag,option) specifies the options.
...
%
%   [u,Du] = Poisson(node,elem,pde,bdFlag) returns also the
...
%
%   [u,Du,eqn] = Poisson(node,elem,pde,bdFlag) returns also the equation
...
%
%   [u,Du,eqn,info] = Poisson(node,elem,pde,bdFlag) returns also the
