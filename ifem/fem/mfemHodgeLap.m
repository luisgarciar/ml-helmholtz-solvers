function [err,time,solver,eqn] = mfemHodgeLap(node,elem,pde,bdFlag,option,varargin)
...
for k = 1:maxIt
    ...
    switch elemType
        case 'ND0'  % lowest order Nedelec element
            [sigma,u,eqn,info] = HodgeLaplacianE(node,elem,pde,bdFlag,option);
        case 'RT0'  % RT0 mixed FEM 
            [sigma,u,eqn,info] = HodgeLaplacianF(node,elem,pde,bdFlag,option);
    end
    % compute error
    t = cputime;
