function [err,time,solver,eqn] = mfemDarcy(node,elem,pde,bdFlag,option,varargin)
...
for k = 1:maxIt
    ...
    switch elemType
        case 'RT0'  % RT0 mixed FEM 
            [p,u,eqn,info] = DarcyRT0(node,elem,pde,bdFlag,option);
        case 'BDM1' % BDM1 mixed FEM
            [p,u,eqn,info] = DarcyBDM1(node,elem,pde,bdFlag,option);
    end
    % compute error
    t = cputime;
    ...
    if isfield(pde,'exactp')
        ...
        % interpolation
        pI = Lagrangeinterpolate(pde.exactu,node,elem,'P0');
...
displaytable(colname,err.N,[],err.h,'%0.2e',err.pL2,'%0.5e',err.pIphL2,'%0.5e',...
