...
%the following vectors represent the 
%segments between vertices
ve1 = node(elem(:,3),:)-node(elem(:,2),:);%P3-P2
ve2 = node(elem(:,1),:)-node(elem(:,3),:);%P1-P3
ve3 = node(elem(:,2),:)-node(elem(:,1),:);%P2-P1
area = 0.5*(-ve3(:,1).*ve2(:,2) + ve3(:,2).*ve2(:,1));

%Gradient  (Dxlambda, Dylambda) of basis functions: 
%(for the computation of these gradients see the notes 
%Chen - Programming the FEM - page 6
%
%nabla lambda = inv(B) nabla hat(lambda) = (Dxlambda, Dylambda)
