function [u, iter, resid] = PFGMRES(A, f, u, maxit, restart, tol, M, print_level)
...
%   tol:            Tolerance
%   M:              Preconditioner (can be a function handle)
%   print_level:    How much information to print (=0: no output; >0 output)
%
% OUTPUT:
%   u:              Solution
%   iter:           Number of iterations
%   resid:          History of l^2 norm of residual
%
% USAGE:
%   [u, iter, resid] = PFGMRES(A, f, u, maxit, restart, tol, [],
%       print_level): PFGMRES without preconditoner
%   [u, iter, resid] = PFGMRES(A, f, u, maxit, restart, tol, M,
...
while (iter < maxit)
    ...
    for i = 1:restart
        ...
        resid(iter) = abs(b(i+1));
        
        % output
        if (print_level > 0)
            fprintf(' %d |  %e  |  %e  | %f |\n', iter-1, resid(iter)/resid(1), resid(iter), resid(iter)/resid(iter-1));
        end
        
        if ((resid(iter)/resid(1)) < tol)
            converge = 1;
            break;
        end
