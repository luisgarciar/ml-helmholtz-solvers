...
%% MGDARCY multigrid solvers for Darcy system discretized by mixed method
%
%      |M  B'| |u|  = |f|
%      |B  0 | |p|  = |g|
%
...
while (max(err(k,:)) > tol) && (k <= maxIt)
    ...
    if printlevel >= 2
        fprintf('#dof: %8.0u,  #nnz: %8.0u, MG Vcycle iter: %2.0u, err = %8.4e\n',...
                 Ndof+N, nnz(M)+nnz(B), k-1, err(k));
    end            
end

err = err(1:k,:);
itStep = k-1;
% find p by solving BB'p = Bf-BMu elementwise
p = zeros(NT,1);
% ubar = zeros(NE,1);
% ubar(freeEdge) = u;
...
if printlevel >= 2
    fprintf('#dof: %8.0u, level: %2.0u,   coarse grid %2.0u, #nnz: %8.0u\n',...
              Ndof+N, level, size(Mi{1},1), nnz(Mi{1}))
end
if printlevel >= 1
    fprintf('#dof: %8.0u,  #nnz: %8.0u, smoothing: %2.0u, iter: %2.0u,   err = %8.4e,   time = %4.2g s\n',...
                 Ndof+N, nnz(M)+nnz(B), mu, itStep, err(end), time)
end
