function [x, error, iter, flag] = gmress( A, x0, b, M, restrt, max_it, tol )
...
%
% input   A        COMPLEX nonsymmetric  matrix
%         x        REAL initial guess vector
%         b        REAL right hand side vector
%         M        REAL right preconditioner matrix
...
for iter = 1:max_it               % begin iteration
    r = b-A*(M\x0);
    V(:,1) = r/norm(r);
    s = norm(r)*e1;               %right hand side of reduced system
    ...
    y =  H(1:m,1:m) \ s(1:m);
    x =  x0 + V(:,1:m)*y;                          % update approximation
